...
class MyWin(QtWidgets.QMainWindow):
    ...
    def open_file(self):
        ...
        with open(filename, 'r') as f:
            ...
            for line in content:
                cnt_str += 1
                indexes1 = re.findall(pattern1, line)
                if indexes1 is not None:
                    for index1 in indexes1:
                        tmp.append('Строка #{0}, позиция #{1}: найдено <{2}>'.format(cnt_str,
                                                                                     line.index(index1),
                                                                                     index1))
                indexes2 = re.findall(pattern2, line)
        ...
        if path == 'script18.log':
            try:
                with open(path, 'a') as f:
                    for line in self._data:
                        print(self._data.index(line), line, '\n')
                        f.write(str(line))
                        f.write('\n')
            except Exception as e:
                print(e.args)
    ...
    def log_check(self):
        ...
        coincidences1 = []
        coincidences2 = []
        pattern_for_name = re.compile("\w{1,}\.txt")
        pattern_for_time = re.compile("\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}")
        pattern_for_coincidences1 = re.compile("Строка #\d+, позиция #\d+: найдено <r'\([0-9]{3}\)\d{3}-\d{2}-\d{2}'>")
        pattern_for_coincidences2 = re.compile("Строка #\d+, позиция #\d+: найдено <r'\([0-9]{3}\)[0-9]{7}'>")
        with open('script18.log', 'r') as f:
            try:
                tmp = f.readlines()
            except Exception as e:
                print('Ошибка в считка', e.args)
            for line in tmp:
                print('LINE: ', line)
                name = re.findall(pattern_for_name, line.replace('\n', ''))
                time = re.findall(pattern_for_time, line.replace('\n', ''))
                coincidence1 = re.findall(pattern_for_coincidences1, line.replace('\n', ''))
                coincidence2 = re.findall(pattern_for_coincidences2, line.replace('\n', ''))
                try:
                    self._data.append((str(name[0]), str(time[0]), coincidence1))
                    self._data.append((str(name[0]), str(time[0]), coincidence2))
                except IndexError as IE:
                    print('Ошибка в индкесации', IE)
            print('#2 ', self._data)
            try:
                self.proccess(type_log=True)
            except Exception as e:
                print('Ошибка в выводе на экран', e.args)
