...
вызова:
reorganize6 --source "D:/Python/Laba2/Music" --days 2 --size 4096
"""
...
def reorganize(source=my_parser.source, days=my_parser.days, size=my_parser.size):
    ...
    for file in files:
        ...
        if tmp + days > now.day:  # проверка на дату создания
            shutil.move(file, source + '\\Archive')
        elif os.path.getsize(file) < size:  # проверка на размер
